...
CREATE TABLE IF NOT EXISTS instruments (
  ...
  instrument_name VARCHAR(255) NOT NULL, serial_number VARCHAR(150) NOT NULL UNIQUE, manufacturer VARCHAR(150), model_number VARCHAR(150),
  ...
  status ENUM('REGISTERED','PENDING_VERIFICATION','VERIFIED','EXPIRED','REJECTED','SUSPENDED') NOT NULL DEFAULT 'REGISTERED',
...
CREATE TABLE IF NOT EXISTS applications (
  ...
  status ENUM('SUBMITTED','UNDER_REVIEW','ASSIGNED','SCHEDULED','INSPECTION_COMPLETED','APPROVED','REJECTED','CANCELLED') NOT NULL DEFAULT 'SUBMITTED',
...
CREATE TABLE IF NOT EXISTS inspections (
  inspection_id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY, application_id INT UNSIGNED NOT NULL, inspector_id INT UNSIGNED NOT NULL, assigned_by_official_id INT UNSIGNED,
  ...
  status ENUM('SCHEDULED','IN_PROGRESS','COMPLETED','CANCELLED') NOT NULL DEFAULT 'SCHEDULED', remarks TEXT,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (application_id) REFERENCES applications(application_id), FOREIGN KEY (inspector_id) REFERENCES inspectors(inspector_id), FOREIGN KEY (assigned_by_official_id) REFERENCES officials(official_id)
) ENGINE=InnoDB;
...
('Pooja','pooja@sih.gov','$2b$10$N9qo8uLOickgx2ZMRZoMyeIjZAgcfl7p92ldGxad68LJZdL17lhWy','INSPECTOR','ACTIVE')
...
INSERT INTO inspectors (user_id,employee_id,designation,department,joining_date) SELECT user_id,'INS-002','Field Inspector','Legal Metrology',CURRENT_DATE FROM users WHERE email='pooja@sih.gov' ON DUPLICATE KEY UPDATE designation=VALUES(designation);
